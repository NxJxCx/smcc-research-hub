import { React, ReactPDF } from "/jsx/imports";
...
export default function PdfViewer({ src }: { src: string }) {
  ...
  return (
    <div className="w-full max-h-fit overflow-hidden bg-gray-300/50 pb-4">
      ...
      <ReactPDF.Document file={src} onLoadSuccess={onDocumentLoadSuccess} onLoadError={(err: any) => console.log("ERROR:", err)}>
        <ReactPDF.Page className="max-w-fit mx-auto overflow-x-auto overflow-y-hidden border drop-shadow-lg"  renderTextLayer={false} renderAnnotationLayer={false} canvasBackground={"#fff"} pageNumber={pageNumber} />
      </ReactPDF.Document>
    </div>
  );
}