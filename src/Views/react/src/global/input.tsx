...
export default import(pathname("/jsx/imports")).then(({ React, clsx }) => {
  function Input({ label, name, type = "text", placeholder = "", className = "", labelColor, inputClassName, ...props }: { label: string, name: string, placeholder?: string, className?: string, type?: React.HTMLInputTypeAttribute, labelColor?: string, inputClassName?: string } & React.InputHTMLAttributes<HTMLInputElement> ) {
  ...
  function Select({ label, name, items, className = "", labelColor, ...props }: { label: string, name: string, items: SelectItem[], className?: string, labelColor?: string } & React.InputHTMLAttributes<HTMLInputElement> ) {
    return (
      <div className={
        clsx(
          "flex flex-col justify-start w-full",
